package com.boat.mpp.stream.sink;
...
import com.boat.mpp.common.domain.SimpleAnchorInfo;
import com.boat.mpp.stream.utils.LettuceRedisUtils;
 ...
 */
@Slf4j
public class MppSink implements SinkFunction<AnchorInfo> {
    ...
    private void realTimeData(AnchorInfo info) {
        try {
            LettuceRedisUtils.pipeline(redisAsyncCommands -> {
                 ...
                 */
                String redisMessageKey = StrUtil.join(StrUtil.COLON, MppConstant.CACHE_KEY_PREFIX, MppConstant.MESSAGE_ID, info.getMessageId());
