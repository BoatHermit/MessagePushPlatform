...
import com.boat.mpp.common.constant.SendAccountConstant;
import com.boat.mpp.common.domain.LogParam;
import com.boat.mpp.common.domain.TaskInfo;
import com.boat.mpp.common.dto.account.DingDingWorkNoticeAccount;
import com.boat.mpp.common.dto.model.DingDingWorkContentModel;
...
public class DingDingWorkNoticeHandler extends BaseHandler implements Handler {
    ...
    public boolean handler(TaskInfo taskInfo) {
        try {
            ...
            OapiMessageCorpconversationAsyncsendV2Response response = new DefaultDingTalkClient(SEND_URL)
                    .execute(request, accessToken);

            // 发送成功后记录TaskId，用于消息撤回(支持当天的)
            if (response.getErrcode() == 0) {
                redisTemplate.opsForList().leftPush(DING_DING_RECALL_KEY_PREFIX + taskInfo.getMessageTemplateId()
                        , String.valueOf(response.getTaskId()));
                redisTemplate.expire(DING_DING_RECALL_KEY_PREFIX + taskInfo.getMessageTemplateId(), (
                        DateUtil.endOfDay(new Date()).getTime() - DateUtil.current()) / 1000, TimeUnit.SECONDS);
                return true;
            }
    ...
    private OapiMessageCorpconversationAsyncsendV2Request assembleParam(
        ...
        try {
            ...
            log.error("assembleParam fail:{},params:{}"
    ...
    public void recall(MessageTemplate messageTemplate) {
        SupportThreadPoolConfig.getPendingSingleThreadPool().execute(() -> {
            try {
                DingDingWorkNoticeAccount account = accountUtils
                        .getAccountById(messageTemplate.getSendAccount(), DingDingWorkNoticeAccount.class);
                String accessToken = redisTemplate.opsForValue()
    ...
    public void pull(Long accountId) {
        try {
            ...
            req.setAgentId(Long.valueOf(account.getAgentId()));
            req.setTaskId(456L);
            OapiMessageCorpconversationGetsendresultResponse rsp
